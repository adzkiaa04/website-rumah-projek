...
<head>
    ...
    <title>REKOMENDASI PROPERTY BARU</title>
    ...
    <style>
        ...
        /* Change text color to white when hovering over the entire card */
        ...
        .searchBox {
            display: flex;
            height: 45px; /* Sesuaikan tinggi agar tidak terlalu panjang ke bawah */
            width: 100%;
            max-width: 300px; /* Lebarkan dari 230px ke 300px */
            align-items: center;
            justify-content: space-between;
            gap: 8px;
            background: #2f3640;
            ...
            overflow: hidden; /* Mencegah isi meluap ke bawah */
            margin-bottom: 50px;
        }

        .searchInput {
            flex: 1; /* Biarkan input memenuhi sisa ruang */
            border: none;
            background: none;
            outline: none;
            color: white;
            font-size: 15px;
            padding: 10px 15px; /* Sesuaikan padding agar tidak kepanjangan */
            width: 100%;
...
<body>
    ...
    <h2>REKOMENDASI PROPERTY BARU</h2>

    <div class="searchBox">
        <input type="text" id="search" class="searchInput" placeholder="Cari rumah...">
    ...
    <section id="rekomendasi">
        <div class="container">
            <?php
            $sql = "SELECT * FROM properti_baru ORDER BY created_at DESC";
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo '
                    <div class="card">
                        <a href="tambah_rumah.php" class="add-button">+</a>
                        <img src="uploads/' . htmlspecialchars($row['gambar']) . '" alt="' . htmlspecialchars($row['nama_rumah']) . '">
                        <div class="card-content">
                            <h3 class="price">IDR ' . number_format($row['harga'], 0, ',', '.') . '</h3>
                            <p class="contact">Hub. ' . htmlspecialchars($row['nomor_kontak']) . '</p>
                            <p class="address">' . htmlspecialchars($row['alamat']) . '</p>
                            <p class="status" style="color: red;">' . htmlspecialchars($row['nama_rumah']) . '</p>
                        </div>
                        <div class="card-footer">
                            <div>
                                <i class="fas fa-bed"></i>
                                <span>' . htmlspecialchars($row['kamar_tidur']) . ' Kamar Tidur</span>
                            </div>
                            <div>
                                <i class="fas fa-bath"></i>
                                <span>' . htmlspecialchars($row['kamar_mandi']) . ' Kamar Mandi</span>
                            </div>
                            <div>
                                <i class="fas fa-ruler-combined"></i>
                                <span>' . htmlspecialchars($row['luas_rumah']) . ' m<sup>2</sup> Luas Rumah</span>
                            </div>
                        </div>
                    </div>';
                ...
                echo '<p>Tidak ada rekomendasi rumah saat ini.</p>';
            }
            ?>
        </div>
    </section>

    <a href="tambah_rumah.php" class="add-button-fixed">+</a>
